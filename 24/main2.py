...
def main():
    ...
    def bfs(start, goal, init_t=0):
        queue = deque([(start[0], start[1], init_t)])
        visited = set([(start[0], start[1], init_t % PERIOD)])
        while queue:
            x, y, t = queue.popleft()
            if (x, y) == goal:
                return t
            ...
            for dx, dy in [(0, 0), *DIR_MAP.values()]:
                ...
                if (nx, ny) in blocked:
                    continue
                if (nx, ny) != start and (nx, ny) != goal:  # allow start and end
    ...
    first_t = bfs(start, end)
    second_t = bfs(end, start, first_t)
    third_t = bfs(start, end, second_t)
    print(third_t)
